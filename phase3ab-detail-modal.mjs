#!/usr/bin/env node
// phase3ab-detail-modal.mjs
// Phase 3a (Detail Modal skeleton) + 3b (Reschedule/Cancel in Jadwal tab)
// Usage: drag ke folder ~/linguo-landing → buka Terminal di situ → `node phase3ab-detail-modal.mjs`
// Script akan auto-commit + push + self-delete kalau sukses.

import fs from 'fs';
import path from 'path';
import { execSync } from 'child_process';

const MODAL_PATH = 'src/components/ClassDetailModal.tsx';
const PAGE_PATH = 'src/app/akun/page.tsx';

// Sanity check
if (!fs.existsSync(PAGE_PATH)) {
  console.error('❌ Tidak menemukan', PAGE_PATH);
  console.error('   Pastikan script ada di folder ~/linguo-landing');
  process.exit(1);
}

console.log('🎯 Phase 3a + 3b: Detail Modal + Reschedule/Cancel');
console.log('─'.repeat(55));

// =========================================================================
// STEP 1 — Create src/components/ClassDetailModal.tsx
// =========================================================================
...
fs.mkdirSync(path.dirname(MODAL_PATH), { recursive: true });
fs.writeFileSync(MODAL_PATH, modalContent);
console.log('✓ Created', MODAL_PATH);

// =========================================================================
// STEP 2 — Modify src/app/akun/page.tsx
// =========================================================================
...
// 2a. Import
if (!page.includes('ClassDetailModal')) {
  const importBlock = page.match(/^(import [\s\S]+?from ['"][^'"]+['"];?\s*\n)+/m);
  if (!importBlock) {
    console.error('❌ Gak bisa nemu block import di page.tsx');
    process.exit(1);
  }
  page = page.replace(importBlock[0], importBlock[0] + `import ClassDetailModal from '@/components/ClassDetailModal';\n`);
  console.log('✓ Added import ClassDetailModal');
}

// 2b. State
if (!page.includes('detailReg')) {
  const stateAnchors = [
    /const \[selectedSlot, setSelectedSlot\] = useState<string \| null>\(null\);/,
    /const \[bookingReg,\s*setBookingReg\][^;]+;/,
    /const \[availSlots,\s*setAvailSlots\][^;]+;/,
  ];
  let injected = false;
  for (const a of stateAnchors) {
    const m = page.match(a);
    if (m) {
      page = page.replace(a, (found) => found + `\n  const [detailReg, setDetailReg] = useState<any>(null);`);
      injected = true;
      console.log('✓ Added detailReg state (anchor: ' + a.source.slice(0, 40) + '…)');
      break;
    }
  }
  if (!injected) {
    const firstUseState = page.match(/const \[[^\]]+\]\s*=\s*useState[^;]+;/);
    if (firstUseState) {
      page = page.replace(firstUseState[0], firstUseState[0] + `\n  const [detailReg, setDetailReg] = useState<any>(null);`);
      console.log('⚠️  Used fallback state injection (after first useState)');
    } else {
      console.error('❌ Gak bisa inject state. Abort.');
      process.exit(1);
    }
  }
}

// 2c. Add "📋 Detail" button right before "Booking Sesi Berikutnya" button
if (!page.includes('setDetailReg(reg)') && !page.includes('setDetailReg(r)')) {
  const bookingTextIdx = page.indexOf('Booking Sesi Berikutnya');
  if (bookingTextIdx > 0) {
    const before = page.slice(0, bookingTextIdx);
    const btnOpenIdx = before.lastIndexOf('<button');
    if (btnOpenIdx > 0) {
      // Detect variable name used (reg vs r)
      const ctxSnippet = page.slice(Math.max(0, btnOpenIdx - 400), btnOpenIdx + 200);
      const usesReg = /\bregistrations?\.map\(\(reg\)/.test(ctxSnippet) || /\(reg\)\s*=>/.test(ctxSnippet);
      const usesR = !usesReg && (/\.map\(\(r\)/.test(ctxSnippet) || /\(r\)\s*=>/.test(ctxSnippet));
      const varName = usesR ? 'r' : 'reg';

      const detailBtn = `<button
                  onClick={(e) => { e.stopPropagation(); setDetailReg(${varName}); }}
                  className="px-3 py-2 rounded-lg text-xs font-semibold bg-gray-100 text-gray-700 hover:bg-gray-200 mr-2"
                >
                  📋 Detail
                </button>
                `;
      page = page.slice(0, btnOpenIdx) + detailBtn + page.slice(btnOpenIdx);
      console.log('✓ Added 📋 Detail button (using var: ' + varName + ')');
    } else {
      console.log('⚠️  Anchor ditemukan tapi <button opening ga ketemu. Skip button injection.');
    }
  } else {
    console.log('⚠️  "Booking Sesi Berikutnya" ga ketemu. Skip button injection.');
    console.log('   Lo bisa manual bikin card clickable dengan `onClick={() => setDetailReg(reg)}`.');
  }
}

// 2d. Render modal at end of JSX
if (!page.includes('<ClassDetailModal')) {
  // Try: inject before the existing BookingModal render `{bookingReg && `
  const bookingRenderMatch = page.match(/\{bookingReg\s*&&\s*\(/);
  if (bookingRenderMatch && bookingRenderMatch.index !== undefined) {
    const injection = `{detailReg && <ClassDetailModal reg={detailReg} onClose={() => setDetailReg(null)} />}\n      `;
    page = page.slice(0, bookingRenderMatch.index) + injection + page.slice(bookingRenderMatch.index);
    console.log('✓ Added <ClassDetailModal /> render (before BookingModal block)');
  } else {
    // Fallback: before closing </div>\n  );\n}
    const endPattern = /(\n\s*<\/div>\s*\n\s*\)\s*;\s*\n\s*\})/;
    if (endPattern.test(page)) {
      page = page.replace(endPattern, `\n      {detailReg && <ClassDetailModal reg={detailReg} onClose={() => setDetailReg(null)} />}$1`);
      console.log('✓ Added <ClassDetailModal /> render (fallback end-of-return)');
    } else {
      console.log('⚠️  Gak bisa auto-inject <ClassDetailModal />. Tambahin manual di akhir return JSX.');
    }
  }
}
...
if (page !== pageOrig) {
  ...
  console.log('✓ Saved', PAGE_PATH);
} else {
  console.log('ℹ️  page.tsx tidak berubah (semua edit udah ada sebelumnya?)');
}

// =========================================================================
// STEP 3 — git commit + push + self-delete
// =========================================================================
...
try {
  ...
  try {
    execSync('git commit -m "feat(akun): Phase 3a+3b — Detail Modal + Reschedule/Cancel in Jadwal tab"', { stdio: 'inherit' });
  } catch {
    console.log('ℹ️  Nothing to commit (no changes).');
  }
  execSync('git push', { stdio: 'inherit' });
  console.log('\n✅ Pushed! Vercel deploy kejalan otomatis.');
  ...
  console.error('   JANGAN delete script ini — cek error dulu di atas.');
}
