...
function CategoryBadge({ category }: { category?: string }) {
  ...
  return <span className={"inline-block px-3 py-0.5 rounded-full text-xs font-medium " + c.bg + " " + c.text}>{category || "Artikel"}</span>;
}
  ...
  return (
    <Link href={"/blog/" + post.slug} className="group bg-white rounded-xl overflow-hidden border border-slate-100 hover:shadow-lg hover:border-[#1A9E9E]/20 transition-all duration-300">
      ...
      <div className="p-4">
        ...
        <h3 className="font-bold text-sm text-slate-900 leading-snug line-clamp-2 group-hover:text-[#1A9E9E] transition-colors">{post.title}</h3>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400 pb-6 border-b border-slate-200">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-[#1A9E9E] flex items-center justify-center text-white font-bold text-xs">L</div>
              <span>By <span className="font-medium text-slate-600">Linguo Team</span></span>
            </div>
        ...
        {/* Body */}
        <article className="prose prose-slate prose-lg max-w-none mb-16 prose-headings:font-extrabold prose-headings:text-slate-900 prose-h2:text-2xl prose-h2:mt-10 prose-h2:mb-4 prose-p:leading-relaxed prose-p:text-slate-600 prose-a:text-[#1A9E9E] prose-a:no-underline hover:prose-a:underline prose-strong:text-slate-900 prose-blockquote:border-l-[#1A9E9E] prose-img:rounded-xl">
        ...
        <div className="bg-[#1A9E9E] rounded-2xl p-8 text-center text-white mb-14">
          ...
          <a href="/" className="inline-block bg-[#fbbf24] hover:bg-[#f59e0b] text-slate-900 font-bold px-8 py-3 rounded-full text-sm transition-colors">Mulai Belajar Sekarang</a>
        </div>
