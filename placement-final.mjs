#!/usr/bin/env node
// Final rebuild PlacementTest:
// 1. Auto-submit on click multiple choice (no "Jawab" button)
// 2. Soft-gate WA di result screen (optional, unlock detail)
// 3. Keep: Indonesian explanations, tips, private class estimation
// Run: cd ~/linguo-landing && node placement-final.mjs
...
if (!fs.existsSync(path.join(ROOT, 'src/app/silabus/[lang]/coba'))) {
  console.error('❌ Run dari ~/linguo-landing');
  process.exit(1);
}

console.log('📝 Final rebuild PlacementTest...\n');

// ============================================================
// PlacementTest.tsx — FINAL VERSION
// ============================================================
...
fs.writeFileSync(ptPath, lines.join('\n'), 'utf8');
console.log('  ✅ PlacementTest.tsx rewritten (auto-click + soft-gate WA)');

// ============================================================
// Update API endpoint to accept name + whatsapp
// ============================================================
const apiPath = path.join(ROOT, 'src/app/api/placement-result/route.ts');
...
console.log('  ✅ API /api/placement-result diperbarui (terima name + whatsapp)');

// ============================================================
// SQL migration: tambah name + whatsapp kolom
// ============================================================
const sqlPath = '/tmp/placement-add-gate-columns.sql';
fs.writeFileSync(sqlPath, `-- Tambah kolom name + whatsapp untuk soft-gate placement result
-- Run di Supabase SQL Editor

ALTER TABLE placement_results ADD COLUMN IF NOT EXISTS name TEXT;
ALTER TABLE placement_results ADD COLUMN IF NOT EXISTS whatsapp TEXT;
CREATE INDEX IF NOT EXISTS idx_placement_whatsapp ON placement_results(whatsapp) WHERE whatsapp IS NOT NULL;

-- Analytics query: unlocked leads dari placement test
-- SELECT language, level, whatsapp, name, score, created_at
...
try {
  ...
  execSync('git commit -m "feat(placement): auto-submit + soft-gate WA for unlocked details"', { stdio: 'inherit', cwd: ROOT });
  execSync('git push', { stdio: 'inherit', cwd: ROOT });
  console.log('\n✅ Pushed\n');
} catch (e) {
  console.log('\n⚠️  Git error:', e.message);
  console.log('Manual push command:');
  console.log('  cd ~/linguo-landing && git add -A && git commit -m "feat(placement): final" && git push');
}
...
console.log(`   1. Run SQL di Supabase: cat ${sqlPath}`);
console.log('   2. Tunggu Vercel deploy (~1 menit)');
console.log('   3. Hard refresh linguo.id/silabus/english/coba');
console.log('   4. Test:');
console.log('      • Klik opsi = jawaban langsung tersubmit (no Jawab button)');
console.log('      • Di result: ada soft-gate "Dapatkan Learning Plan Gratis"');
console.log('      • Input nama + WA → detail chapter + estimasi unlock');
console.log('═══════════════════════════════════════════════\n');
