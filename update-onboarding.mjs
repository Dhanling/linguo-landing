      ...
      <div className="max-w-lg mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 mt-3 bg-[#1A9E9E]/5 rounded-xl px-4 py-2.5 text-xs">
            ...
            <span className="text-slate-600">{programLabel}</span>
            <span className="text-slate-300">•</span>
            ...
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              ...
              {livesInIndonesia === true && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Provinsi</label>
              ...
              {livesInIndonesia === false && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Negara</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Kota</label>
                    <input type="text" placeholder="contoh: Singapore, Tokyo, Dubai" value={cityAbroad} onChange={(e) => setCityAbroad(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-[#1A9E9E] focus:ring-2 focus:ring-[#1A9E9E]/20" />
                  </div>
                </motion.div>
              )}
              ...
              <button onClick={() => {
                ...
                if (livesInIndonesia === null) { setError("Pilih lokasi tinggal"); return; }
                if (livesInIndonesia && (!province || !city.trim())) { setError("Lengkapi provinsi dan kota"); return; }
                if (!livesInIndonesia && (!country || !cityAbroad.trim())) { setError("Lengkapi negara dan kota"); return; }
                setError(""); setStep(2);
              }}
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              <div>
                ...
                {livesInIndonesia === false && (
                  <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2 mt-2">Jadwal di bawah menggunakan zona waktu WIB (GMT+7)</p>
                )}
              </div>
