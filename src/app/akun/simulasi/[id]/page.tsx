...
import {
  ...
  XCircle, Lightbulb, MessageCircle, Target, MinusCircle, GraduationCap,
} from "lucide-react";
...
// JAWABAN SISWA (indeks pilihan / teks / rekaman) supaya di halaman hasil bisa
// ditampilkan berdampingan dengan kunci jawaban.
type ResultItem = {
...
// ── Result ──────────────────────────────────────────────────────────────────
// Nomor WA admin — pesan dari CTA di bawah masuk ke WA Inbox dashboard.
const WA_ADMIN = "6282116859493";

const optLetter = (i: number) => String.fromCharCode(65 + i);

// Teks satu opsi (sudah dibersihkan dari label "(A)" bawaan soal).
function optionTextOf(q: Question, idx: number | null): string | null {
...
function keyTextOf(q: Question): string | null {
...
function ReviewCard({ r, no }: { r: ResultItem; no: number }) {
  const q = r.question;
...
function ResultView({ sim, sections, totals, results, preview, studentName }: {
  sim: Simulation;
  sections: Section[];
  ...
  const waText = encodeURIComponent(
    `Halo admin Linguo, saya ${studentName || "calon siswa"}. Saya baru selesai ${sim.title} (${testTypeLabel(sim.test_type, sim.test_variant)}) dengan skor ${Math.round(totals.score)}/${Math.round(totals.max_score)} (${pct}%). Saya mau tanya kelas persiapan privat untuk menaikkan skor saya.`
  );
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {preview ? (
            ...
            <>
              <Link href="/akun/simulasi" className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-600">
