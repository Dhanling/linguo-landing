...
const CAT_COLORS: Record<string,{bg:string;text:string;darkBg:string;darkText:string}> = {
  Tips:{bg:"bg-violet-100",text:"text-violet-600",darkBg:"bg-violet-900/40",darkText:"text-violet-300"},
  Edukasi:{bg:"bg-rose-100",text:"text-rose-600",darkBg:"bg-rose-900/40",darkText:"text-rose-300"},
  Fun:{bg:"bg-emerald-50",text:"text-emerald-600",darkBg:"bg-emerald-900/40",darkText:"text-emerald-300"},
  Grammar:{bg:"bg-pink-100",text:"text-pink-600",darkBg:"bg-pink-900/40",darkText:"text-pink-300"},
};
const GRADS = [
...
function Badge({cat,dm}:{cat?:string;dm:boolean}){
  const c=cat&&CAT_COLORS[cat]?CAT_COLORS[cat]:{bg:"bg-slate-100",text:"text-slate-600",darkBg:"bg-slate-700",darkText:"text-slate-300"};
...
function ClapButton({slug,dm}:{slug:string;dm:boolean}){
  ...
  return(
    <button onClick={(e)=>{e.preventDefault();e.stopPropagation();clap()}} className={`flex items-center gap-1 transition-colors ${dm?"text-slate-500 hover:text-slate-300":"text-slate-400 hover:text-slate-600"}`} title="Clap">
...
function BookmarkBtn({slug,dm}:{slug:string;dm:boolean}){
  const [saved,setSaved]=useState(false);
  useEffect(()=>setSaved(isSaved(slug)),[slug]);
  return(
    <button onClick={(e)=>{e.preventDefault();e.stopPropagation();setSaved(toggleSave(slug))}} className={`transition-colors ${saved?"text-[#1A9E9E]":dm?"text-slate-600 hover:text-slate-400":"text-slate-300 hover:text-slate-500"}`} title={saved?"Unsave":"Save"}>
      <Bookmark className={`w-[18px] h-[18px] ${saved?"fill-current":""}`}/>
    </button>
  )
}

function MoreMenu({slug,onHide,dm}:{slug:string;onHide:()=>void;dm:boolean}){
  ...
  return(
    <div className="relative" ref={ref}>
      <button onClick={(e)=>{e.preventDefault();e.stopPropagation();setOpen(!open)}} className={`transition-colors ${dm?"text-slate-600 hover:text-slate-400":"text-slate-300 hover:text-slate-500"}`}>
        <MoreHorizontal className="w-5 h-5"/>
      </button>
      {open&&(
        <div className={`absolute right-0 top-8 border rounded-xl shadow-xl py-1 w-52 z-50 ${dm?"bg-[#1e293b] border-slate-700":"bg-white border-slate-200"}`}>
          <button onClick={(e)=>{e.preventDefault();e.stopPropagation();hidePost(slug);onHide();setOpen(false)}} className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 ${dm?"text-slate-300 hover:bg-slate-700":"text-slate-600 hover:bg-slate-50"}`}>
            <X className="w-4 h-4"/> Not interested
          </button>
          <button onClick={(e)=>{e.preventDefault();e.stopPropagation();navigator.clipboard?.writeText(`https://linguo.id/blog/${slug}`);setOpen(false)}} className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 ${dm?"text-slate-300 hover:bg-slate-700":"text-slate-600 hover:bg-slate-50"}`}>
...
function FeedItem({post,onHide,dm}:{post:BlogPost;onHide:()=>void;dm:boolean}){
    ...
    <article className={`py-7 border-b last:border-0 ${dm?"border-slate-800":"border-slate-100"}`}>
      ...
      <Link href={"/blog/"+post.slug} className="group flex gap-6">
        <div className="flex-1 min-w-0">
          <h2 className={`text-[22px] font-extrabold leading-snug mb-1 group-hover:text-[#1A9E9E] transition-colors line-clamp-2 ${dm?"text-slate-100":"text-[#0f172a]"}`}>{post.title}</h2>
          <p className={`text-[15px] leading-relaxed line-clamp-2 mb-0 hidden sm:block ${dm?"text-slate-400":"text-slate-600"}`}>{exc}</p>
        </div>
        <div className="w-28 h-28 sm:w-36 sm:h-32 shrink-0 rounded-lg overflow-hidden">
          <Cover post={post} sz="sm" cls="group-hover:scale-110 transition-transform duration-300"/>
        </div>
      </Link>
      <div className="flex items-center justify-between mt-4">
        <div className={`flex items-center gap-4 text-xs ${dm?"text-slate-500":"text-slate-400"}`}>
        ...
        <div className="flex items-center gap-3">
          <ClapButton slug={post.slug} dm={dm}/>
          <Link href={"/blog/"+post.slug} className={`flex items-center gap-1 transition-colors ${dm?"text-slate-600 hover:text-slate-400":"text-slate-300 hover:text-slate-500"}`} title="Comments">
            <MessageCircle className="w-[18px] h-[18px]"/>
            <span className="text-xs">{post.view_count||0}</span>
          </Link>
          <BookmarkBtn slug={post.slug} dm={dm}/>
          <MoreMenu slug={post.slug} onHide={onHide} dm={dm}/>
        </div>
      </div>
    </article>
  )
...
function StaffPicks({posts,dm}:{posts:BlogPost[];dm:boolean}){return(
  <div className="mb-8">
    <h3 className={`text-[15px] font-extrabold mb-4 ${dm?"text-slate-100":"text-[#0f172a]"}`}>Staff Picks</h3>
    <div className="space-y-5">
      {posts.slice(0,3).map(p=>(
        <Link key={p.id} href={"/blog/"+p.slug} className="group block">
          ...
          <p className={`text-[11px] mt-1.5 flex items-center gap-1 ${dm?"text-slate-500":"text-slate-400"}`}><span className="text-[#1A9E9E]">★</span>{fmtD(p.published_at)}</p>
        </Link>
      ))}
    </div>
  </div>
...
function RecTopics({active,onSelect,dm}:{active:string;onSelect:(c:string)=>void;dm:boolean}){return(
  <div className="mb-8">
    <h3 className={`text-[15px] font-extrabold mb-3 ${dm?"text-slate-100":"text-slate-900"}`}>Recommended Topics</h3>
    <div className="flex flex-wrap gap-2">
      {CATEGORIES.filter(c=>c!=="Semua").map(c=>{
        const cl=CAT_COLORS[c]||{bg:"bg-slate-100",text:"text-slate-600",darkBg:"bg-slate-700",darkText:"text-slate-300"};
        const isActive=active===c;
        return <button key={c} onClick={()=>onSelect(isActive?"Semua":c)}
          className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition-all border ${
            isActive
              ? dm?cl.darkBg+" "+cl.darkText+" border-transparent":cl.bg+" "+cl.text+" border-current/20"
              : dm?"bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700":"bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100"
          }`}>{c}</button>
      })}
    </div>
  </div>
...
function GridCard({post,dm}:{post:BlogPost;dm:boolean}){
  const mins=rt(post.content);
  return(
    <Link href={"/blog/"+post.slug} className={`group rounded-xl overflow-hidden border hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col ${dm?"bg-[#1e293b] border-slate-700 hover:border-slate-500":"bg-white border-slate-100 hover:border-slate-200"}`}>
      ...
      <div className="p-5 flex flex-col flex-1">
        <Badge cat={post.category} dm={dm}/>
        <h3 className={`font-extrabold text-[17px] leading-snug mt-2 mb-2 line-clamp-2 group-hover:text-[#1A9E9E] transition-colors ${dm?"text-slate-100":"text-[#0f172a]"}`}>{post.title}</h3>
        ...
        <div className={`flex items-center justify-between pt-3 border-t ${dm?"border-slate-700":"border-slate-50"}`}>
          ...
          <span className={`text-xs flex items-center gap-0.5 ${dm?"text-slate-500":"text-slate-400"}`}><Clock className="w-3 h-3"/>{mins} min</span>
        </div>
      </div>
    </Link>
  )
...
function SmCard({post,dm}:{post:BlogPost;dm:boolean}){return(
  <Link href={"/blog/"+post.slug} className={`group flex gap-3 p-2.5 rounded-xl border hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${dm?"bg-[#1e293b] border-slate-700 hover:border-[#1A9E9E]/30":"bg-white border-slate-100 hover:border-[#1A9E9E]/20"}`}>
    ...
    <div className="flex flex-col justify-center min-w-0 flex-1">
      <Badge cat={post.category} dm={dm}/>
      <h3 className={`text-[12px] font-bold leading-snug line-clamp-2 mt-1 group-hover:text-[#1A9E9E] transition-colors ${dm?"text-slate-200":"text-slate-900"}`}>{post.title}</h3>
      <span className={`text-[10px] mt-1 ${dm?"text-slate-500":"text-slate-400"}`}>{fmtD(post.published_at)}</span>
    </div>
  </Link>
)}

function RankCard({post,rank,dm}:{post:BlogPost;rank:number;dm:boolean}){return(
  <Link href={"/blog/"+post.slug} className={`group flex items-center gap-3 p-2.5 rounded-xl transition-all ${dm?"hover:bg-slate-800":"hover:bg-slate-50"}`}>
    <span className={`text-2xl font-black w-7 text-center shrink-0 group-hover:text-[#1A9E9E]/60 ${dm?"text-[#1A9E9E]/20":"text-[#1A9E9E]/15"}`}>{rank}</span>
...
function CircCard({post,dm}:{post:BlogPost;dm:boolean}){return(
  <Link href={"/blog/"+post.slug} className={`group flex gap-3 items-center p-2.5 rounded-xl transition-all ${dm?"hover:bg-slate-800":"hover:bg-slate-50"}`}>
    <div className={`w-12 h-12 shrink-0 rounded-full overflow-hidden ring-2 group-hover:ring-[#1A9E9E]/30 ${dm?"ring-slate-700":"ring-slate-100"}`}><Cover post={post} sz="sm"/></div>
    <div className="flex flex-col min-w-0 flex-1">
      <Badge cat={post.category} dm={dm}/>
      <h3 className={`text-[12px] font-bold leading-snug line-clamp-2 mt-1 group-hover:text-[#1A9E9E] transition-colors ${dm?"text-slate-200":"text-slate-900"}`}>{post.title}</h3>
      <span className={`text-[10px] mt-1 ${dm?"text-slate-500":"text-slate-400"}`}>{fmtD(post.published_at)}</span>
...
function Pagi({cur,tot,go,dm}:{cur:number;tot:number;go:(p:number)=>void;dm:boolean}){
  ...
  return <div className="flex items-center justify-center gap-1.5 my-8">
    {cur>1&&<button onClick={()=>go(cur-1)} className={`w-9 h-9 rounded-full flex items-center justify-center ${dm?"text-slate-400 hover:bg-slate-800":"text-slate-400 hover:bg-slate-100"}`}><ChevronLeft className="w-4 h-4"/></button>}
    {pgs.map((p,i)=>typeof p==="string"?<span key={"d"+i} className={`px-1 text-sm ${dm?"text-slate-600":"text-slate-300"}`}>…</span>:
      <button key={p} onClick={()=>go(p)} className={p===cur?"w-9 h-9 rounded-full bg-[#1A9E9E] text-white font-bold text-sm shadow-sm":`w-9 h-9 rounded-full font-medium text-sm ${dm?"text-slate-400 hover:bg-slate-800":"text-slate-500 hover:bg-slate-100"}`}>{p}</button>)}
...
export default function BlogContent({initialPosts}:{initialPosts:BlogPost[]}){
  ...
  useEffect(()=>{
    ...
    setHiddenSlugs(h);
    try{setDm(localStorage.getItem("blog_dm")==="1")}catch{}
  },[initialPosts]);

  const toggleDm=()=>{
    const next=!dm;
    setDm(next);
    try{localStorage.setItem("blog_dm",next?"1":"0")}catch{}
  };
  ...
  return(
    <div className={`min-h-screen transition-colors duration-300 ${dm?"bg-[#0f172a]":"bg-white"}`} style={{fontFamily:"'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,sans-serif"}}>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative hidden md:block">
              ...
              <input type="text" placeholder="Search" value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}
                className={`w-52 pl-9 pr-3 py-1.5 rounded-full border-0 focus:ring-1 outline-none text-sm transition-all ${dm?"bg-slate-800 text-slate-200 placeholder-slate-600 focus:bg-slate-700 focus:ring-slate-600":"bg-slate-50 text-slate-800 focus:bg-white focus:ring-slate-200"}`}/>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            <div className={`flex items-center rounded-full p-0.5 ${dm?"bg-slate-800":"bg-slate-100"}`}>
              <button onClick={()=>setView("feed")} className={`p-1.5 rounded-full transition-all ${view==="feed"?(dm?"bg-slate-600 text-[#1A9E9E]":"bg-white shadow-sm text-[#1A9E9E]"):dm?"text-slate-500 hover:text-slate-300":"text-slate-400 hover:text-slate-600"}`} title="Feed"><List className="w-4 h-4"/></button>
              <button onClick={()=>setView("grid")} className={`p-1.5 rounded-full transition-all ${view==="grid"?(dm?"bg-slate-600 text-[#1A9E9E]":"bg-white shadow-sm text-[#1A9E9E]"):dm?"text-slate-500 hover:text-slate-300":"text-slate-400 hover:text-slate-600"}`} title="Grid"><LayoutGrid className="w-4 h-4"/></button>
            </div>
            {/* Dark mode toggle */}
            <button
              onClick={toggleDm}
              title={dm?"Kembali ke light mode":"Aktifkan dark mode"}
              className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 ${dm?"bg-slate-700 text-yellow-300 hover:bg-slate-600 hover:text-yellow-200":"bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700"}`}
            >
      ...
      {/* TABS */}
      <div className={`border-b transition-colors duration-300 ${dm?"bg-[#111827] border-slate-800":"bg-white border-slate-100"}`}>
      ...
      <div className="md:hidden px-6 pt-3">
        <div className="relative">
          <Search className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${dm?"text-slate-600":"text-slate-400"}`}/>
          <input type="text" placeholder="Cari artikel..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}
            className={`w-full pl-10 pr-4 py-2.5 rounded-full border outline-none text-sm ${dm?"bg-slate-800 border-slate-700 text-slate-200 placeholder-slate-600":"bg-white border-slate-200 text-slate-800"}`}/>
        </div>
      </div>

      {filtered.length===0?<div className="text-center py-20"><p className={`text-lg ${dm?"text-slate-600":"text-slate-400"}`}>Belum ada artikel.</p></div>:(
      <>
        {/* FEED VIEW */}
        {view==="feed"&&(
          <div className="max-w-[1200px] mx-auto px-6 pt-2">
            <div className="flex gap-16">
              <div className="flex-1 min-w-0 max-w-[728px]">
                {feedVis.map(p=><FeedItem key={p.id} post={p} onHide={()=>doHide(p.slug)} dm={dm}/>)}
                ...
                {feedN>=filtered.length&&filtered.length>0&&<p className={`text-center text-sm py-10 ${dm?"text-slate-700":"text-slate-300"}`}>— Semua artikel sudah ditampilkan —</p>}
              </div>
              <aside className={`hidden lg:block w-[340px] shrink-0 border-l pl-10 pt-8 ${dm?"border-slate-800":"border-slate-100"}`}>
                <div className="sticky top-20">
                  <StaffPicks posts={picks} dm={dm}/>
        ...
        {view==="grid"&&(
          <div className="max-w-7xl mx-auto px-6 py-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-4">{gridPg.map(p=><GridCard key={p.id} post={p} dm={dm}/>)}</div>
            <Pagi cur={page} tot={totP} go={p=>{setPage(p);window.scrollTo({top:0,behavior:"smooth"})}} dm={dm}/>
            {page===1&&!search&&filtered.length>=2&&(
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-4">
                <div>
                  <h2 className={`text-base font-extrabold mb-3 flex items-center gap-2 ${dm?"text-slate-100":"text-slate-900"}`}><span className="w-1 h-4 bg-[#1A9E9E] rounded-full"/>Artikel Terbaru</h2>
                  <div className="space-y-2">{terbaru.map(p=><SmCard key={p.id} post={p} dm={dm}/>)}</div>
                </div>
                <div>
                  <h2 className={`text-base font-extrabold mb-3 flex items-center gap-2 ${dm?"text-slate-100":"text-slate-900"}`}><span className="w-1 h-4 bg-[#fbbf24] rounded-full"/>Artikel Populer</h2>
                  <div className="space-y-2">{picks.map((p,i)=><RankCard key={p.id} post={p} rank={i+1} dm={dm}/>)}</div>
                </div>
                <div>
                  <h2 className={`text-base font-extrabold mb-3 flex items-center gap-2 ${dm?"text-slate-100":"text-slate-900"}`}><span className="w-1 h-4 bg-rose-400 rounded-full"/>Direkomendasikan</h2>
                  <div className="space-y-2">{rekom.map(p=><CircCard key={p.id} post={p} dm={dm}/>)}</div>
                </div>
              </div>
            )}
          </div>
