...
// linguo-patch:placement-bundle-ramping-v1 — bank soal & kurikulum dipilih di server
// (page.tsx), klien cuma terima SATU bahasa. Dulu semua bank soal + seluruh data
// silabus ikut ke bundle browser -> klik bahasa di picker loading lama.
export default function CobaClient({
  curriculum,
  questions,
}: {
  curriculum: LanguageCurriculum | null;
  questions: PlacementQuestion[] | null;
}) {
  if (curriculum && questions) {
...
function IeltsToeflPlacement() {
  ...
  async function submitLead(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("leads").insert({
      ...
      wa_number: wa.trim(),
      language: `${testLabel}`, // ielts-lead-prog-v2: kolom Bahasa
      program: "ielts-toefl", // ielts-lead-prog-v2: kolom Program -> label "IELTS/TOEFL Prep"
