...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt =
      ...
      `          "parts": ordered text segments that concatenate EXACTLY to the full ${language} conjugated form, WITH all spaces included inside the segment text so the pieces join into a naturally-spaced phrase (e.g. for English "I tethered" use [{"t":"I ","c":false},{"t":"tether","c":false},{"t":"ed","c":true}] — note the trailing space after the pronoun; NEVER let a subject word and the verb run together into one token; for reflexive verbs the reflexive pronoun is its own segment WITH its space, e.g. French "Nous nous inquiétons" → [{"t":"Nous ","c":false},{"t":"nous ","c":true},{"t":"inquiét","c":false},{"t":"ons","c":true}]). Each { "t": the segment text, "c": true ONLY for the segment(s) that CHANGE across the paradigm (the inflected prefix/suffix), false for the invariant stem and for any subject pronoun or space },\n` +
