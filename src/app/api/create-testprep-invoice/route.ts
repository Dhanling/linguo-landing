...
// sampai ke akarnya (external_id ditanam di satu baris). Di sini:
//   - external_id invoice = `LINGUO-TP-<ts>-<rand>` (baris pertama);
//   - baris ke-2 dst pakai sufiks `-2`, `-3` (kolom itu kunci 1:1 di banyak
//     tempat, jangan sampai dua baris berbagi satu external_id);
//   - webhook mengenali prefix LINGUO-TP- lalu memperlakukan semua baris
//     seawalan itu sebagai satu tagihan (lihat xendit-webhook).
// Total dihitung ULANG di server dari quoteCartItem — `amount` dari client
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const raw of rawItems) {
      ...
      seen.add(k);
      const priced = quoteCartItem(it);
      if (!priced) continue;
      items.push({ ...it, ...priced });
    }
    if (!items.length) {
    ...
    const externalId = `LINGUO-TP-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    ...
    if (!leadRes.ok) {
      ...
      console.error("Lead insert error (testprep cart):", err);
    ...
    const description = items.length === 1
      ...
      : `Persiapan Ujian Linguo — ${items.length} paket: ${items.map((it) => `${it.product.test} ${it.level} (${it.format === "semi" ? "semi-private" : "private"})`).join(", ")}`;
    ...
    if (!xenditRes.ok) {
      ...
      console.error("Xendit error (testprep cart):", err);
      let detail = err;
    ...
    console.error("create-testprep-invoice error:", error);
