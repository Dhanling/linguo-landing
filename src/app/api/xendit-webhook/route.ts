...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/leads?${leadFilter}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      if (isMultiLeadCart) {
        ...
        try {
          const sibRes = await fetch(
            `${SUPABASE_URL}/rest/v1/leads?${leadFilter}&select=xendit_external_id&order=xendit_external_id.asc`,
            { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
          );
          const sibs: { xendit_external_id: string }[] = sibRes.ok ? await sibRes.json() : [];
