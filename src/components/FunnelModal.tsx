...
export default function FunnelModal({open,onClose,initialProgram="",initialLang="",initialLevel="",initialPreferredProg="",initialSource="",initialName="",initialWa=""}:{open:boolean;onClose:()=>void;initialProgram?:string;initialLang?:string;initialLevel?:string;initialPreferredProg?:string;initialSource?:string;initialName?:string;initialWa?:string}) {
  ...
  useEffect(() => {
    ...
    if (!open) { setStep(1); setSelProgram(""); setSelLang(""); setSelLevel(""); setSelTeacherType("lokal"); setTeacherPick(false); setSelDuration(60); setSelSessions(12); setClassMode("online"); setOfflineCity(""); }
  ...
  // offline-private-class-v1 — Kelas Private bisa offline (pengajar datang ke
  // tempat siswa): tarif online + selisih tetap per sesi. Kids tetap online.
  const canOffline = supportsOffline(selProgram);
  const isOffline = canOffline && classMode==="offline";
  const perSessionOnline = selProgram==="Kelas Private" ? privatePerSession : selProgram==="Kelas Kids" ? kidsPerSession : 0;
  ...
  // offline-private-class-v1 — pemilih mode kelas. Ditulis sebagai fungsi (bukan
  // komponen) supaya input kota tidak remount & kehilangan fokus tiap ketikan.
