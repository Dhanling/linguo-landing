...
// linguo-patch:ling-lesson-reposition-v1  — geser bubble/panel ke atas di /akun/belajar biar ga nutupin tombol Selesaikan
...
export default function ChatWidget() {
  ...
  // [ling-lesson-reposition-v1] di player LMS, footer punya tombol Selesaikan di pojok kanan bawah.
  // Angkat bubble + panel ke bottom-24 cuma di route ini biar ga ketutup. Route lain tetap bottom-5.
  const pathname = usePathname();
  const onLesson = pathname === "/akun/belajar";
  const bubbleBottom = onLesson ? "bottom-24" : "bottom-5";
    ...
    <>
      {!open && (
        <button
          ...
          className={`fixed ${bubbleBottom} right-5 z-[9998] flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg transition-transform hover:scale-105 active:scale-95`}
        >
      ...
      {open && (
        <div style={{ fontFamily: FONT }} className={`fixed ${bubbleBottom} right-5 z-[9999] flex h-[70vh] max-h-[560px] w-[92vw] max-w-[380px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10`}>
